...
06 - broke privilege analysis into separate file
...
TTITLE "Database Security Risk Assessment (Privileges)"
...
spool dbra_privileges_$ORACLE_SID.out

SELECT '********** Database Security Risk Assessment Privileges '||sysdate||' FOR DATABASE: '||name||' **********' FROM v$database;
set heading on
...
select * from 
(select count(*) "COUNT_SYSPRIVS" , grantee "USER", 'USER' "WHAT"
  from dba_sys_privs, DBA_users
  where username = grantee
 group by grantee
 UNION
 select count(*) "COUNT_SYSPRIVS" , grantee "USER", 'ROLE' "WHAT"
...
/
PROMPT  **************END OF RISK ASSESSMENT PRIVILEGE ANALYSIS*******************
PROMPT  **************************************************************************
set verif on
set lines 80
spool off
